...
from model import FusionTLTRClassifierRegressorModel, TLTRModel


from trainer import ClassifierRegressorTrainer
...
@click.command()
@click.option('-p', '--config_path', default='/home/adas/Projects/Wav_LM/Fusion/cls_reg_config.yml', type=str)
...
def main(config_path):
    ...
    model = FusionTLTRClassifierRegressorModel(whisper_model, wavlm_model, w2v_model)
    model.to(device)
    ...
    trainer = ClassifierRegressorTrainer(args=Munch(config['loss_params']),
                      model=model,
                      optimizer=optimizer,
                      device=device,
